...
class AdvHybridTrainer:
    ...
    def train(self, epochs: int, logger) -> None:
        ...
        for epoch in range(1, epochs + 1):
            logger.info(f"========== Epoch {epoch}/{epochs} (adv-hybrid) ==========")
            ...
            # epoch 结束后做一次评估
            metrics_recon, metrics_fore, metrics_hybrid = self.evaluate(epoch, logger)
            ...
            if f1 > self.best_f1 + 1e-6:
                ...
                best_path = os.path.join(self.log_dir, "best_model_adv_hybrid.pt")
                torch.save(self.model.state_dict(), best_path)
                logger.info(f"发现更优模型，已保存至 {best_path}")

        logger.info(
            f"训练结束，最佳混合 F1={self.best_f1:.4f}, 最佳指标={self.best_metrics}"
        )
    ...
    def _train_epoch_plain(self, epoch: int, logger) -> None:
        ...
        if n_batches > 0:
            logger.info(
                f"[Epoch {epoch}] 训练损失(plain): "
                f"total={total_loss_sum / n_batches:.6f}, "
                f"recon={rec_loss_sum / n_batches:.6f}, "
                f"forecast={pred_loss_sum / n_batches:.6f}"
            )
    ...
    def _train_epoch_stage1(self, epoch: int, logger) -> None:
        ...
        for batch in self.train_loader:
            ...
            with autocast(enabled=self.use_amp):
                ...
                # 构造负样本：用预测结果替换真实未来
                x_neg = torch.cat(
                    [x[:, : self.context_len, :], y_pred], dim=1
                )  # [B, L, D]
            ...
            if not torch.isfinite(loss):
                logger.warning("遇到非有限 loss (NaN/Inf)，跳过该 batch (stage1)")
                continue
        ...
        if n_batches > 0:
            logger.info(
                f"[Epoch {epoch}] 阶段1对抗训练损失: "
                f"total={total_loss_sum / n_batches:.6f}, "
                f"recon_pos={rec_loss_sum / n_batches:.6f}, "
                f"pred={pred_loss_sum / n_batches:.6f}, "
                f"recon_neg={adv_loss_sum / n_batches:.6f}"
            )
    ...
    def _train_epoch_stage2(self, epoch: int, logger) -> None:
        ...
        for batch in self.train_loader:
            ...
            if not torch.isfinite(loss):
                logger.warning("遇到非有限 loss (NaN/Inf)，跳过该 batch (stage2)")
                continue
    ...
    def evaluate(self, epoch: int, logger) -> Tuple[Dict, Dict, Dict]:
        """
        ...
        with torch.no_grad():
            ...
            for batch in self.test_loader:
                ...
                rec_err = ((x_hat - x) ** 2).mean(dim=(1, 2)).cpu().numpy()  # [B]
        ...
        # 先分别搜索各自的最佳阈值
        thr_recon, metrics_recon = search_best_f1_threshold(
            recon_z, labels_all, use_point_adjust=self.use_point_adjust
        )
        thr_fore, metrics_fore = search_best_f1_threshold(
            forecast_z, labels_all, use_point_adjust=self.use_point_adjust
        )

        logger.info(
            "[Eval-recon(adv-hybrid)] "
            f"F1={metrics_recon['f1']:.4f}, P={metrics_recon['precision']:.4f}, "
            f"R={metrics_recon['recall']:.4f}, AUC={metrics_recon['auc']:.4f}, "
            f"thr={metrics_recon['threshold']:.6f}, point_adjust={self.use_point_adjust}"
        )
        logger.info(
            "[Eval-forecast(adv-hybrid)] "
            f"F1={metrics_fore['f1']:.4f}, P={metrics_fore['precision']:.4f}, "
            f"R={metrics_fore['recall']:.4f}, AUC={metrics_fore['auc']:.4f}, "
            f"thr={metrics_fore['threshold']:.6f}, point_adjust={self.use_point_adjust}"
        )
        ...
        thr_h, metrics_hybrid = search_best_f1_threshold(
            hybrid_scores, labels_all, use_point_adjust=self.use_point_adjust
        )

        logger.info(
            "[Eval-hybrid(adv)] "
            f"F1={metrics_hybrid['f1']:.4f}, P={metrics_hybrid['precision']:.4f}, "
            f"R={metrics_hybrid['recall']:.4f}, AUC={metrics_hybrid['auc']:.4f}, "
            f"thr={metrics_hybrid['threshold']:.6f}, point_adjust={self.use_point_adjust}; "
            f"recon_F1={metrics_recon['f1']:.4f}, forecast_F1={metrics_fore['f1']:.4f}"
        )

        # 画一张 hybrid 得分的可视化图（使用第一条实体的得分）
        first_scores = hybrid_scores[: len(labels_list[0])]
